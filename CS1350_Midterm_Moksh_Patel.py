...
def find_overcommitted(events, max_events):
    ...
    for event in events:
        for volunteer in event.volunteers:
            count[volunteer] = count.get(volunteer, 0) + 1
    return {v for v, c in count.items() if c > max_events}
...
class Matrix:
    ...
    def __str__(self):
        return "\n".join("".join(f"{val:4}" for val in row) for row in self.rows)
    ...
    def _total(self):
        return sum(val for row in self.rows for val in row)
